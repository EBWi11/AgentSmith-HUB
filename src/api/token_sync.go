...
// WriteLLMConfigToRedis writes LLM API key and base URL to Redis (called by leader when config is set).
// Ensures all nodes in the cluster use the same LLM config.
func WriteLLMConfigToRedis(apiKey, baseURL string) error {
	if apiKey == "" {
		return nil
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
...
// ReadLLMConfigFromRedis reads LLM API key and base URL from Redis (called by followers).
// Overrides local config so cluster-wide LLM config is consistent.
func ReadLLMConfigFromRedis() (apiKey, baseURL string, err error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	client := common.GetRedisClient()
