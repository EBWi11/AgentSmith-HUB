...
// calculateDailyMessageCounts calculates the real message counts for today (from 00:00) (internal method)
func (qm *QPSManager) calculateDailyMessageCounts(projectID string) map[string]interface{} {
	...
	for sequenceKey, componentData := range sequenceGroups {
		...
		// Calculate daily message count (messages since start of today)
		var dailyMessages uint64 = 0
		...
		if len(componentData.DataPoints) >= 1 {
			// Find the earliest data point for today or use the latest point from yesterday as baseline
			var baselineMessages uint64 = 0
			var foundBaselineForToday bool = false

			// Look for data points from today to establish baseline
			for _, point := range componentData.DataPoints {
				if point.Timestamp.After(startOfDay) {
					// This is from today, use as baseline if it's the earliest
					if !foundBaselineForToday {
						baselineMessages = point.TotalMessages
						foundBaselineForToday = true
					} else if point.TotalMessages < baselineMessages {
						baselineMessages = point.TotalMessages
					}
				}
			}

			// If we found data from today, calculate the difference
			if foundBaselineForToday {
				currentMessages := componentData.CurrentTotal
				if currentMessages >= baselineMessages {
					dailyMessages = currentMessages - baselineMessages
				} else {
					// Handle restart case: use current total as daily count
					dailyMessages = currentMessages
				}
			} else {
				// No data from today, assume all current messages are from today
				dailyMessages = componentData.CurrentTotal
			}
		} else {
			// No data points, use current total
			dailyMessages = componentData.CurrentTotal
		}
		...
		result[sequenceKey] = map[string]interface{}{
			...
			"project_node_sequence": sequenceKey,
			"total_messages":        totalMessages, // Real cumulative total
			"daily_messages":        dailyMessages, // Messages since start of today
			"data_points_count":     len(componentData.DataPoints),
		}
	}
...
// calculateAggregatedDailyMessages calculates aggregated message counts across all nodes for today (internal method)
func (qm *QPSManager) calculateAggregatedDailyMessages() map[string]interface{} {
	...
	totalInputMessages := uint64(0)                    // Total daily input messages across all projects
	totalOutputMessages := uint64(0)                   // Total daily output messages across all projects
	totalRulesetMessages := uint64(0)                  // Total daily ruleset processed messages
	...
	for _, data := range qm.data {
		...
		if len(data.DataPoints) >= 1 {
			// Find the earliest data point for today or use current total as baseline
			var baselineMessages uint64 = 0
			var foundBaselineForToday bool = false
			...
			// If we found data from today, calculate the difference
			if foundBaselineForToday {
				currentMessages := data.CurrentTotal
				if currentMessages >= baselineMessages {
					dailyMessages = currentMessages - baselineMessages
				} else {
					// Handle restart case: use current total as daily count
		...
		// Count daily messages for individual statistics
		projectStats[data.ProjectID][data.ComponentType] += dailyMessages

		// For global totals, count daily messages by component type
		parts := strings.Split(data.ProjectNodeSequence, ".")
		...
		case "ruleset":
			// Count ruleset processing - this shows daily processing count
			totalRulesetMessages += dailyMessages
		}
	}

	return map[string]interface{}{
		"total_messages":         totalInputMessages + totalOutputMessages + totalRulesetMessages, // All daily messages
		"total_input_messages":   totalInputMessages,                                              // Daily input messages
		"total_output_messages":  totalOutputMessages,                                             // Daily output messages
		"total_ruleset_messages": totalRulesetMessages,                                            // Daily ruleset processed messages
		"project_breakdown":      projectStats,                                                    // Includes all component daily counts
	}
}

// calculateNodeDailyMessages calculates message counts by node for today (internal method)
func (qm *QPSManager) calculateNodeDailyMessages() map[string]interface{} {
	...
	for _, data := range qm.data {
		...
		// Count daily messages for node statistics - shows actual message processing count
	...
	for nodeID, stats := range nodeStats {
		inputMessages := stats["input"]
		outputMessages := stats["output"]
		rulesetMessages := stats["ruleset"] // Daily ruleset processing count

		result[nodeID] = map[string]interface{}{
			"input_messages":   inputMessages,
			"output_messages":  outputMessages,
			"ruleset_messages": rulesetMessages,                                  // Daily ruleset processing count
			"total_messages":   inputMessages + outputMessages + rulesetMessages, // Total daily message processing count
		}
	}
...
// GetDailyMessageCounts returns message counts for today
func (qm *QPSManager) GetDailyMessageCounts(projectID string) map[string]interface{} {
	return qm.calculateDailyMessageCounts(projectID)
}

// GetAggregatedDailyMessages returns aggregated message counts across all nodes for today
func (qm *QPSManager) GetAggregatedDailyMessages() map[string]interface{} {
	return qm.calculateAggregatedDailyMessages()
}

// GetNodeDailyMessages returns message counts for a specific node for today
func (qm *QPSManager) GetNodeDailyMessages(nodeID string) map[string]interface{} {
	allNodeData := qm.calculateNodeDailyMessages()
	if nodeData, exists := allNodeData[nodeID]; exists {
		return nodeData.(map[string]interface{})
	}
